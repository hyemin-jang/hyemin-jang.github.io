...
- `<class>model.entity.Customer</class>` : Entity 클래스를 등록
- 각 속성(property)의 의미
...
![image-20210821133801987](C:\development\githubpage\my-blog\content\assets\posting images\image-20210821133801987.png)
...
public class RunningTest {
	...
	void test() {
        ...
        // 엔티티 클래스의 setter 메소드 - tx.commit() 시에 UPDATE문 수행 
            ...
            // SELECT one
            Member oneMember = em.find(Member.class, 1L);  // pk값(1L)으로 조회
		    ...
		    // DELETE
            em.remove(m);  // 객체를 파라미터로 받아서 pk값을 통해 해당 데이터 삭제
